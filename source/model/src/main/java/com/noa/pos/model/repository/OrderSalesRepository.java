...
public interface OrderSalesRepository extends JpaRepository<OrderSalesEntity, Long> {
    ...
    @Query("SELECT new com.noa.pos.model.dto.ReportOrderSalesProdDto(os.orderSalesId ,os.ticketNumber, os.typePayment, p.productId, p.name, p.price, os.dateRegister, SUM(osd.quantity))" +
            " from OrderSalesEntity os, OrderSalesDetailEntity osd, ProductEntity p where os.orderSalesId = osd.orderSalesId" +
    ...
    @Query("SELECT sa from OrderSalesEntity sa where sa.lastTime between :from and :to and sa.state = 'PREPARING' and sa.companyId = :companyId " +
            "union SELECT sa from OrderSalesEntity sa where sa.lastTime between :from and :to and sa.state = 'PROCESSED' and sa.companyId = :companyId")
    List<OrderSalesEntity> findByLastTimeBetweenPending(LocalDateTime from, LocalDateTime to, Integer companyId);
}
